...
//		Object accID = 21;
		...
		try {
			...
			for (w = 0; w < sourceSystems.getLength(); w++) {
				...
				if (systmName.equals(sourceSystemName)) {
							...
							for (int l = 0; l < acc_Entities.getLength(); l++) {
								...
								if (systemName.equals(sourceSystemName)) {
									...
									for (int m = 0; m < acc_attribute.getLength(); m++) {
										...
										// for accountAddress
										NodeList acc_add_Entities = sourceSystem
										...
										for (int r = 0; r < acc_add_Entities.getLength(); r++) {
											...
											if (systemName.equals(sourceSystemName)) {
												...
												for (int n = 0; m < acc_add_attri.getLength(); m++) {
													acc_add_att = (Element) acc_add_attri.item(m);

													attributes = acc_add_att.getElementsByTagName("Attribute");
													for (int k = 0; k < attributes.getLength(); k++) {
														// System.out.println("Node Type:"+
														// attributes.item(k).getNodeType());
														if (attributes.item(k).getNodeType() != Node.ELEMENT_NODE) {
															continue;
														}
														Element attribute = (Element) attributes.item(k);
...
//										System.out.println("Here--------------------------");
										...
										// For Contact
										NodeList con_Entities = sourceSystem
												.getElementsByTagName("RecordSnapshot_Queries");
										...
										for (int j = 0; j < con_Entities.getLength(); j++) {
											...
											if (systemName.equals(sourceSystemName)) {
												NodeList con_attri = sourceSystem
														.getElementsByTagName("ChildEntity_Contact");
												...
												for (int p = 0; p < con_add_Entities.getLength(); p++) {
													...
													// con_WhereClause =
													// con_Entity.getAttribute("ContactIdWhereClause");
													...
													if (systemName.equals(sourceSystemName)) {
														...
														for (int n = 0; n < con_add_attri.getLength(); n++) {
															...
															for (int k = 0; k < attributes.getLength(); k++) {
																if (attributes.item(k)
																		.getNodeType() != Node.ELEMENT_NODE) {
																	continue;
																}
																Element attribute = (Element) attributes.item(k);
																if (false == attribute.getNodeName()
																		.equals("Attribute")) {
																	break;
																}
																String sourceName = attribute
																		.getAttribute("SourceName");
																// System.out.println("SourceName:"+sourceName);
																String targetName = attribute
											...
											while (rs1.next()) {
												...
												for (int z = 1; z <= numColumns; z++) {
													...
													json_account.put(acc_fields.get(column_name),
															rs1.getObject(column_name));
...
////										System.out.println("next:"+ acc_add_fields.get(colum_name));
														json_account_address.put(acc_add_fields.get(colum_name),
																rs4.getObject(colum_name));
													}
...
//													System.out.println("value:"+ con_add_fields.get(colum_name));
											...
											PrintWriter outFile = new PrintWriter(new FileWriter(
													"AccID_" + accId + "_" + formatter.format(date) + ".json"));
